...
def read_data(input_filename):
    ...
    ws_in = df.loc[:, "wind"]
    prep_in = df.loc[:, "precipitation10m"]
...
def draw(lons,
    ...
    if opt_addrain:
        ...
        # clevs = [0.5, 1., 3., 5., 10., 15., 20., 30., 2000.]
        ccols = [
            "lavender", "paleturquoise", "dodgerblue", "b", "gold",
            "darkorange", "r", "firebrick"
        ]
        # collevsクラスの初期化（降水量の範囲をclevs、色をccolsで与える）
        r2c = collevs(clevs=clevs, ccols=ccols)
        #
        # テキストをプロット
        for xc, yc, dc in zip(lons, lats, prep):
            if math.isnan(dc):
                print("Warn ", xc, yc, dc)
            else:
                c = r2c.conv(dc)
...
def main(tinfo=None,
    ...
    if tinfof is not None:
        ...
        varname = "_temp"
        if opt_barbs:
            varname = varname + "+wind"
        if opt_addrain:
            varname = varname + "+cumrain"
        output_filename = os.path.join(output_dir,
                                       area + varname + "_" + tinfof + ".png")
        # データの取得
        lons, lats, temp, u, v, pr_i = read_data(input_filename)
        if ofirst:
            prep = pr_i
        else:
            prep = prep + pr_i
