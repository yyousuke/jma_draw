...
catopy_jma_temp+wind.py（風を矢羽、気温をマーカーでプロット）

catopy_jma_temp+wind+rain.py（風を矢羽、気温をマーカー、降水量をテキストでプロット）
...
- **開始・終了時刻**：それぞれtime_sta、time_end。指定すると、その時刻範囲内で作図
...
- **気温の範囲**：カラーバーに設定する気温の範囲は下限：tmin、上限：tmax、ラベルの値を描く間隔はtstepで与える

- **作図する時刻間隔や矢羽を描く値**：変更する場合は、プログラム中の次の部分を変更する必要がある

作図する時刻間隔はtime_stepで指定している
...
- **作図する時刻間隔**：変更する場合は、プログラム中の次の部分を変更する必要がある
...
- **開始・終了時刻**：それぞれtime_sta、time_end。指定すると、その時刻範囲内で開始時刻からの積算降水量を作図
...
- **取得する降水量データ**：変更する場合は、プログラム中の次の部分を変更する必要がある

amedas.read_data(rainstep="1h")の時刻間隔を変える
